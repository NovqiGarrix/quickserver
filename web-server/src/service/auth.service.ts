...
async function loginWithGoogle(code: string): Promise<{ ok: boolean; message: string, data: { accessToken: string, refreshToken?: string } }> {
    ...
    if(existUser) {
        await ioRedis.set(sessionId, 960, JSON.stringify(existUser)); // 1 minutes longer in redis
        const accessToken = jwt.signToken({ email: existUser.email, sessionId }, { expiresIn: '15m' });
    ...
    await ioRedis.set(sessionId, 960, JSON.stringify(newUser)); // 1 minutes longer in redis

    const accessToken = jwt.signToken({ email: newUser.email, sessionId }, { expiresIn: '15m' });
